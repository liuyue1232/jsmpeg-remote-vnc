...
#define FF_COMPRESSION_DEFAULT -1
     ...
     */
    AVRational time_base; //表示时间戳的基本时间单位
...
typedef struct AVCodec {
    ...
    enum AVMediaType type; //指明类型，视频or音频or字幕
    enum AVCodecID id; //ID，这里为MPEG1
    /**
     * Codec capabilities.
     * see CODEC_CAP_*
     */
    int capabilities;
    const AVRational *supported_framerates; //仅视频 array of supported framerates, or NULL if any, array is terminated by {0,0}
    const enum AVPixelFormat *pix_fmts;     //仅视频，支持的像素格式，这里是AV_PIX_FMT_YUV420P
    const int *supported_samplerates;       //仅音频 array of supported audio samplerates, or NULL if unknown, array is terminated by 0
    const enum AVSampleFormat *sample_fmts; //仅音频 array of supported sample formats, or NULL if unknown, array is terminated by -1
    const uint64_t *channel_layouts;         ///< array of support channel layouts, or NULL if unknown. array is terminated by 0
#if FF_API_LOWRES
    uint8_t max_lowres;                     ///< maximum value for lowres supported by the decoder, no direct access, use av_codec_get_max_lowres()
#endif
    const AVClass *priv_class;              //私有数据的大小 AVClass for the private context
